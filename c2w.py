...
# Set this to your browser's executable path if ChromeDriverManager can't find it
# or if you want to use a specific browser (e.g., Brave).
# Example for Brave:
# BROWSER_BINARY_LOCATION = r"C:\\Users\\RAJESH\\AppData\\Local\\BraveSoftware\\Brave-Browser\\Application\\brave.exe"
BROWSER_BINARY_LOCATION = None # Set to None to let ChromeDriverManager find default Chrome/Chromium

# Viewport height for each screenshot. Adjust if needed.
# This impacts how many screenshots are taken and how tall each one is in the document.
VIEWPORT_HEIGHT = 1394 # Pixels


def run_code(file_path: str) -> Tuple[str | None, str | None]:
    """
    Executes a given code file and captures its output and errors.
...
def generate_full_page_screenshots(file_path: str, viewport_height: int = VIEWPORT_HEIGHT) -> List[str]:
    """
    ...
    driver.set_window_size(1920, viewport_height) # Set initial viewport height
    driver.get(file_url)
    time.sleep(2) # Give page time to load and render content
    ...
    while current_scroll_position < page_height:
        ...
        screenshot_name = f"{os.path.splitext(abs_path)[0]}_screenshot_{screenshot_counter:02d}.png"
        ...
        time.sleep(0.5) # Small pause for rendering after scroll

        # Re-check page height in case dynamic content loaded
        page_height = driver.execute_script("return document.body.scrollHeight")
        
        # If we scrolled past the actual content, we're done
        if current_scroll_position >= page_height and screenshot_counter > 1:
            ...
            # If the next scroll would go past the end, scroll to the very bottom
            driver.execute_script(f"window.scrollTo(0, {page_height});")
            time.sleep(0.5) # Small pause for rendering after scroll
            # Ensure we take the final screenshot
            screenshot_name = f"{os.path.splitext(abs_path)[0]}_screenshot_{screenshot_counter:02d}.png"
            driver.save_screenshot(screenshot_name)
            screenshot_paths.append(screenshot_name)
            break # All done
...
def create_word_doc(files: List[str], metadata: Dict[str, str], output_dir: str = "."):
    ...
    for i, file_path in enumerate(files):
        ...
        if ext in ['.html', '.css', '.js']:
            try:
                # Call the new function for full page screenshots
                ...
                if screenshot_paths:
                    for s_path in screenshot_paths:
                        ...
                        # Clean up screenshot files after adding to doc
                        os.remove(s_path)
                else:
            ...
            try:
                output_text, error_text = run_code(file_path)
    ...
    output_docx = os.path.join(output_dir, f"{metadata['name'].split()[0]}_{metadata['subject']}_{metadata['experiment_no']}.docx")
